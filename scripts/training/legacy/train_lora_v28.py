# -*- coding: utf-8 -*-
"""
LoRA Training v28 - BOPBTL Mask + Multi-Loss

Changes from v27:
  1. Training mask: full mask -> BOPBTL detected mask (train-inference consistency)
  2. Loss: MSE -> MSE + LPIPS + SSIM
  3. Eval images: img7, 14, 100 + selected
"""
...
BOPBTL_DIR = os.path.join(THIRD_PARTY_DIR, "Bringing-Old-Photos-Back-to-Life", "Global")
sys.path.insert(0, BOPBTL_DIR)
sys.path.insert(0, os.path.join(BOPBTL_DIR, "detection_models"))
...
# ======================================================================
# BOPBTL Mask Generation
# ======================================================================

def load_bopbtl_model(device="cpu"):
    from detection_models import networks
    model = networks.UNet(
        in_channels=1, out_channels=1, depth=4, conv_num=2, wf=6,
        padding=True, batch_norm=True, up_mode="upsample",
        with_tanh=False, sync_bn=True, antialiasing=True,
    )
    ckpt_path = os.path.join(BOPBTL_DIR, "checkpoints", "detection", "FT_Epoch_latest.pt")
    ckpt = torch.load(ckpt_path, map_location="cpu")
    model.load_state_dict(ckpt["model_state"])
    model.to(device).eval()
    log("BOPBTL detection model loaded")
    return model


@torch.no_grad()
def bopbtl_detect(model, image_pil, device="cpu"):
    import torchvision as tv
    ow, oh = image_pil.size
    img = image_pil.convert("L")
    img = tv.transforms.ToTensor()(img)
    img = tv.transforms.Normalize([0.5], [0.5])(img)
    img = img.unsqueeze(0)
    _, _, w, h = img.shape
    scale = 256 / min(w, h)
    ...
    nw = max(nw, 16)
    nh = max(nh, 16)
    img_scaled = F.interpolate(img, [nh, nw], mode="bilinear")
    img_scaled = img_scaled.to(device)
    P = torch.sigmoid(model(img_scaled))
    P = P.data.cpu()
    P = F.interpolate(P, [h, w], mode="nearest")
    mask = (P >= 0.4).float().squeeze().numpy()
    mask_uint8 = (mask * 255).astype(np.uint8)
    mask_pil = Image.fromarray(mask_uint8).resize((ow, oh), Image.NEAREST)
    return mask_pil


def preprocess_bopbtl_masks(pairs, cache_dir, device="cpu"):
    ...
    for dmg_path, clean_path, fname in pairs:
        ...
        result.append((mask_path, fname))
        if not os.path.exists(mask_path):
            need_process.append((dmg_path, mask_path, fname))

    if not need_process:
        log(f"All {len(pairs)} BOPBTL masks already cached in {cache_dir}")
        return result

    log(f"Generating BOPBTL masks for {len(need_process)} images...")
    model = load_bopbtl_model(device)

    for dmg_path, mask_path, fname in tqdm(need_process, desc="BOPBTL masks"):
        try:
            img = Image.open(dmg_path).convert("RGB")
            mask = bopbtl_detect(model, img, device)
            mask.save(mask_path, "PNG")
        except Exception as e:
            log(f"Error generating mask for {fname}: {e}", "WARN")
    ...
    log(f"BOPBTL mask preprocessing done. {len(need_process)} masks cached.")
    return result


# ======================================================================
# SwinIR Preprocessing (same as v27)
# ======================================================================
...
def load_swinir_model(weights_path=None, device="cuda"):
    ...
    if weights_path and os.path.exists(weights_path):
        ...
        weights_path = huggingface_hub.hf_hub_download(
            repo_id="lxq007/DiffBIR-v2", filename="realesrgan_s4_swinir_100k.pth",
            local_dir="weights",
        )
        state_dict = torch.load(weights_path, map_location="cpu", weights_only=True)
...
def preprocess_swinir(pairs, cache_dir, weights_path=None, device="cuda"):
    ...
    if not need_process:
        log(f"All {len(pairs)} SwinIR outputs already cached")
        return swinir_pairs

    log(f"Preprocessing {len(need_process)} images with SwinIR...")
    model = load_swinir_model(weights_path, device)

    for dmg_path, swinir_path, fname in tqdm(need_process, desc="SwinIR"):
        try:
    ...
    gc.collect()
    torch.cuda.empty_cache()
    log(f"SwinIR preprocessing done. {len(need_process)} images cached.")
    return swinir_pairs
...
class LPIPSLoss(torch.nn.Module):
    def __init__(self, device="cuda"):
        ...
        try:
            ...
            self.available = True
            log("LPIPS model loaded (alex)")
        except ImportError:
            log("LPIPS not available, using fallback", "WARN")
    ...
    def forward(self, pred, target):
        ...
        pred_f = (pred.float() + 1) / 2
        target_f = (target.float() + 1) / 2
        return self.model(pred_f, target_f).mean()
...
def ssim_loss(pred, target, window_size=7):
    ...
    mu_pred = F.conv2d(pred_f, kernel, groups=channels, padding=window_size // 2)
    mu_target = F.conv2d(target_f, kernel, groups=channels, padding=window_size // 2)

    mu_pred_sq = mu_pred ** 2
    mu_target_sq = mu_target ** 2
    mu_cross = mu_pred * mu_target

    sigma_pred_sq = F.conv2d(pred_f ** 2, kernel, groups=channels, padding=window_size // 2) - mu_pred_sq
    sigma_target_sq = F.conv2d(target_f ** 2, kernel, groups=channels, padding=window_size // 2) - mu_target_sq
    sigma_cross = F.conv2d(pred_f * target_f, kernel, groups=channels, padding=window_size // 2) - mu_cross
...
def evaluate(pipe, test_data, prompt, strength_values=[0.15, 0.2, 0.25, 0.3, 0.4], seed=42):
    ...
    for strength in strength_values:
        results = {"psnr": [], "ssim": []}
        for swinir_path, clean_path, mask_path, _ in test_data:
            try:
...
def train(train_data, test_data, config):
    ...
    total_steps = config["epochs"] * len(train_data) // config["grad_acc"]
    from diffusers.optimization import get_scheduler
    lr_scheduler = get_scheduler("cosine", optimizer, num_warmup_steps=config["warmup"], num_training_steps=total_steps)
    ...
    log(f"Training: {len(train_data)} pairs, {config['epochs']} epochs, starting from epoch {start_epoch + 1}")
    log(f"Pipeline: SwinIR (preprocessed) + BOPBTL Mask -> SDXL Inpainting + LoRA")
    log(f"Loss: MSE (with SNR Gamma={config['snr_gamma']})")
    ...
    for epoch in range(start_epoch, config["epochs"]):
        ...
        for swinir_path, clean_path, mask_path, _ in pbar:
            try:
                clean = Image.open(clean_path).convert("RGB").resize((512, 512), Image.LANCZOS)
                ...
                mask_t = (torch.from_numpy(np.array(mask_img)).float().unsqueeze(0).unsqueeze(0) / 255.0).cuda().half()
                ...
                mask_lat = F.interpolate(mask_t, size=(64, 64), mode="nearest")
                ...
                with torch.amp.autocast('cuda'):
                    pred = unet(torch.cat([noisy, swinir_lat, mask_lat], 1), t,
                ...
                log(f"Training step error: {e}", "WARN")
                continue
        ...
        if (epoch + 1) % config["eval_every"] == 0:
            ...
            result = evaluate(eval_pipe, test_data, config["prompt"],
...
def main():
    parser = argparse.ArgumentParser(description="LoRA Training v28 - BOPBTL Mask + Multi-Loss")
    ...
    parser.add_argument("--output_dir", type=str, default=str(CHECKPOINTS_DIR / "lora_inpainting_v28"))
    parser.add_argument("--swinir_cache_dir", type=str, default=str(CACHE_DIR / "swinir"))
    parser.add_argument("--mask_cache_dir", type=str, default=str(CACHE_DIR / "bopbtl_masks"))
    ...
    config = {
        ...
        "pipeline": "swinir_bopbtl_sdxl_lora",
    ...
    log("=" * 60)
    log("LoRA Training v28 - BOPBTL Mask + Multi-Loss")
    log("=" * 60)
    for k, v in config.items():
        log(f"  {k}: {v}")

    EVAL_NAMES = ["lrp_img7", "lrp_img14", "lrp_img100", "lrp_img47", "lrp_img78", "lrp_img165"]
    ...
    all_pairs = [(os.path.join(degraded_dir, df), os.path.join(clean_dir, df), df)
                 for df in degraded_files if df in clean_files]
    log(f"Found {len(all_pairs)} total pairs")

    test_pairs = [p for p in all_pairs if p[2].replace(".jpg", "") in EVAL_NAMES]
    if len(test_pairs) < len(EVAL_NAMES):
        log(f"Warning: only found {len(test_pairs)}/{len(EVAL_NAMES)} eval images")
    train_pairs = [p for p in all_pairs if p not in test_pairs]
    log(f"Train: {len(train_pairs)}, Test: {len(test_pairs)}")

    log("=" * 60)
    log("Stage 0: BOPBTL Mask Preprocessing")
    log("=" * 60)
    train_mask_info = preprocess_bopbtl_masks(
        train_pairs, os.path.join(args.mask_cache_dir, "train"), device="cuda"
    )
    test_mask_info = preprocess_bopbtl_masks(
        test_pairs, os.path.join(args.mask_cache_dir, "test"), device="cuda"
    )
    ...
    train_swinir_pairs = preprocess_swinir(
        train_pairs, os.path.join(args.swinir_cache_dir, "train"),
        weights_path=args.swinir_weights, device="cuda"
    )
    test_swinir_pairs = preprocess_swinir(
        test_pairs, os.path.join(args.swinir_cache_dir, "test"),
        weights_path=args.swinir_weights, device="cuda"
    )

    train_data = []
    for (swinir_path, clean_path, fname), (mask_path, _) in zip(train_swinir_pairs, train_mask_info):
        train_data.append((swinir_path, clean_path, mask_path, fname))

    test_data = []
    for (swinir_path, clean_path, fname), (mask_path, _) in zip(test_swinir_pairs, test_mask_info):
        test_data.append((swinir_path, clean_path, mask_path, fname))

    log("=" * 60)
    log("Stage 2: SDXL Inpainting + LoRA Training (with BOPBTL masks)")
    log("=" * 60)
