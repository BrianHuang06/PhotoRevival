# -*- coding: utf-8 -*-
"""
Photo Restoration Training - ControlNet + LoRA v2

Approach:
  1. SDXL Base (frozen) generates images
  2. ControlNet Canny (LoRA fine-tuned) extracts structure from degraded image
  3. Training: clean latent + noise, canny(degraded) as condition -> predict noise
  4. Inference: degraded latent + noise (img2img), canny(degraded) as condition -> restore

Why this should work better than inpainting:
  - Canny edges capture structure, not texture/color defects
  - ControlNet guides UNet to preserve structure while restoring details
  - No full-mask confusion (model won't try to "repaint" everything)
"""
...
BASE_MODEL_ID = str(BASE_MODELS_DIR / "sdxl-base")
CONTROLNET_ID = str(BASE_MODELS_DIR / "controlnet-canny-sdxl")
...
OUTPUT_DIR = str(CHECKPOINTS_DIR / "lora_controlnet_v2_512")
TEST_OUTPUT_DIR = str(ARTIFACTS_DIR / "evaluations" / "controlnet_v2_512")
LOG_FILE = str(LOGS_DIR / "training_log_controlnet_v2_512.json")
...
def prepare_conditioning_image(img_pil, size=1024):
    import cv2
    img = np.array(img_pil.resize((size, size), Image.LANCZOS))
...
def evaluate(pipe, pairs, name="base", strength=0.3, guidance_scale=7.5, lpips_fn=None):
    ...
    for idx, (dmg_path, clean_path, fname) in enumerate(pairs):
        try:
            ...
            cond_img = prepare_conditioning_image(damaged_img, size=RESOLUTION)
            ...
            init_img = damaged_img.resize((RESOLUTION, RESOLUTION), Image.LANCZOS)
            ...
            if idx < 3:
                ...
                cond_img.save(os.path.join(TEST_OUTPUT_DIR, f"{name}_cond_{fname}"))
...
def train(train_pairs, test_pairs, config, lpips_fn):
    ...
    log(f"\n{'='*60}")
    log(f"Training - ControlNet + LoRA v2")
    ...
    controlnet = ControlNetModel.from_pretrained(
        CONTROLNET_ID, torch_dtype=torch.float16, variant="fp16", use_safetensors=True
    )
    ...
    vae = AutoencoderKL.from_pretrained(
        BASE_MODEL_ID, subfolder="vae", torch_dtype=torch.float16,
        variant="fp16", use_safetensors=True
    )

    # Add LoRA to ControlNet
    log("Adding LoRA to ControlNet...")
    ...
    params = sum(p.numel() for p in controlnet.parameters() if p.requires_grad)
    log(f"Trainable LoRA params: {params:,}")

    vae_scaling_factor = 0.13025  # SDXL default
    ...
    optimizer = torch.optim.AdamW(
        controlnet.parameters(), lr=config["learning_rate"],
    ...
    tokenizer_2_out = tokenizer_2(
        PROMPT, padding="max_length", max_length=tokenizer_2.model_max_length,
        truncation=True, return_tensors="pt"
    )

    with torch.no_grad():
        emb1 = text_encoder(tokenizer_out.input_ids.to("cuda"))[0]
        emb2 = text_encoder_2(tokenizer_2_out.input_ids.to("cuda"), output_hidden_states=True)
        emb2_hidden = emb2.hidden_states[-2]
        pooled = emb2.text_embeds

    text_emb = torch.cat([emb1, emb2_hidden], dim=-1)
    ...
    for epoch in range(config["num_epochs"]):
        ...
        for dmg_path, clean_path, fname in pbar:
            try:
                clean_pil = Image.open(clean_path).convert("RGB").resize((RESOLUTION, RESOLUTION), Image.LANCZOS)
                dmg_pil = Image.open(dmg_path).convert("RGB").resize((RESOLUTION, RESOLUTION), Image.LANCZOS)
                ...
                # Prepare conditioning image (canny from degraded)
                cond_pil = prepare_conditioning_image(dmg_pil, size=RESOLUTION)

                # Encode images to latents
                clean_img_t = torch.from_numpy(
                    np.array(clean_pil).astype(np.float32) / 127.5 - 1.0
                ).permute(2, 0, 1).unsqueeze(0).cuda().half()

                cond_img_t = torch.from_numpy(
                    np.array(cond_pil).astype(np.float32) / 127.5 - 1.0
                ).permute(2, 0, 1).unsqueeze(0).cuda().half()

                with torch.no_grad():
                    clean_latent = vae.encode(clean_img_t).latent_dist.sample() * vae_scaling_factor
                    cond_latent = vae.encode(cond_img_t).latent_dist.sample() * vae_scaling_factor

                # Add noise to clean latent
                noise = torch.randn_like(clean_latent)
                t = torch.randint(0, noise_scheduler.config.num_train_timesteps, (1,), device="cuda")
                noisy = noise_scheduler.add_noise(clean_latent, noise, t)

                # ControlNet conditioning (needs 3-channel RGB, not latent)
                cond_dict = {"text_embeds": pooled, "time_ids": time_ids}
                down_block_res_samples, mid_block_res_sample = controlnet(
                    noisy, t,
                    encoder_hidden_states=text_emb,
                    added_cond_kwargs=cond_dict,
                    controlnet_cond=cond_img_t,
                    return_dict=False,
                )

                # UNet forward (frozen params, but need grad graph for ControlNet backprop)
                unet_out = unet(
                    noisy, t,
                    encoder_hidden_states=text_emb,
                    added_cond_kwargs=cond_dict,
                ...
                # Loss: predict noise (compute in fp32 for stability)
                loss = F.mse_loss(unet_out.float(), noise.float())
                ...
                del clean_latent, cond_latent, noisy, noise, down_block_res_samples, mid_block_res_sample, unet_out
        ...
        if (epoch + 1) % 5 == 0:
            ...
            eval_controlnet = ControlNetModel.from_pretrained(
                CONTROLNET_ID, torch_dtype=torch.float16, variant="fp16", use_safetensors=True
            )
            eval_controlnet = PeftModel.from_pretrained(eval_controlnet, best_path)

            eval_pipe = StableDiffusionXLControlNetPipeline.from_pretrained(
                BASE_MODEL_ID, controlnet=eval_controlnet,
            ...
            for strength in [0.3, 0.5]:
                r = evaluate(
                    ...
                    strength=strength, guidance_scale=7.5, lpips_fn=lpips_fn
                )
...
def main():
    log("=" * 60)
    log("Photo Restoration - ControlNet + LoRA v2")
    log("=" * 60)
    ...
    # Evaluate base model
    log("\n" + "=" * 60)
    log("Step 0: Evaluate Base SDXL + ControlNet")
    log("=" * 60)
    ...
    pipe = StableDiffusionXLControlNetPipeline.from_pretrained(
        BASE_MODEL_ID, controlnet=base_controlnet,
        torch_dtype=torch.float16, use_safetensors=True,
        variant="fp16", low_cpu_mem_usage=True
    )
    ...
    from peft import PeftModel
    final_controlnet = ControlNetModel.from_pretrained(
        CONTROLNET_ID, torch_dtype=torch.float16, variant="fp16", use_safetensors=True
    )
    final_controlnet = PeftModel.from_pretrained(final_controlnet, best_path)
    lora_pipe = StableDiffusionXLControlNetPipeline.from_pretrained(
        BASE_MODEL_ID, controlnet=final_controlnet,
        torch_dtype=torch.float16, use_safetensors=True,
        variant="fp16", low_cpu_mem_usage=True
    )
